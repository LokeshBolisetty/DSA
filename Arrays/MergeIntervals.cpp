...
vector<Interval> MergeIntervals(vector<Interval> intervals,Interval newInterval){
    ...
    bool set=false;
    for(int i =0;i<n;i++){
        if(intervals[i].start<newInterval.start)continue;
    ...
    if(!set)intervals.push_back(newInterval);
    vector<Interval> answer;
    for(int i =0;i<n+1;i++){
        Interval a(intervals[i].start,intervals[i].end);
        while(i<intervals.size()-1 && a.end>=intervals[i+1].start){
            if(intervals[i+1].end>a.end)a.end = intervals[i+1].end;
            i++;
        }
        answer.push_back(a);
    }
